...
class CpusmallDataModule(L.LightningDataModule):

    def __init__(self, data_dir: str = os.getenv("LIBSVM_DIR"), batch_size: int = 32):
        super().__init__()

        data, target = sklearn.datasets.load_svmlight_file(f'{data_dir}/cpusmall')
...
class CpusmallRegressor(BaseTrainingModule):
    ...
    def build_model(self):
        return RegressionModel(input_dim=self.input_dim)
...
def run_experiment(config):
    ...
    data_module = CpusmallDataModule(batch_size=config['batch_size'])
    data_module.setup('fit')

    seed_everything(config['seed'], workers=True)

    model = CpusmallRegressor(input_dim=data_module.num_features, config=config)
