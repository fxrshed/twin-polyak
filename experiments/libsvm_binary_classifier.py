...
from npkit.trainer import (
    NumpyBaseTrainingModule, 
    NumpyLogger,
    CSVLoggerCallback
    )
...
class NumpyLibSVMBinaryClassifier(NumpyBaseTrainingModule):
    ...
    def __init__(self, 
        ...
        logger = NumpyLogger(save_dir=f'logs/{dataset_name}', 
                             version=datetime.datetime.now().strftime("%Y%m%d-%H%M%S")) 
        super().__init__(logger=logger, max_epochs=max_epochs)
        ...
        if self.config['optimizer'] == 'STP':
            config.get('optimizer_hparams', {}).update(
                self.optimizer[0].defaults,
            )
        else:
            config.get('optimizer_hparams', {}).update(
        ...
        self.callbacks = [CSVLoggerCallback(self)]
    ...
    def training_step(self, batch):
        ...
        if self.config['optimizer'] == 'STP':
            ...
            if h_x > h_y:
                ...
                self.logger.log('train_loss', loss_y, 
                                epoch=self.current_epoch, step=self.current_step, 
                                on_epoch=True)
                self.logger.log('lr', self.optimizer[0].lr,
                                epoch=self.current_epoch, step=self.current_step,
                                 on_epoch=False)
            else:
                loss_diff = h_y - h_x 
                self.optimizer[1].step(loss_diff)
                self.logger.log('train_loss', loss_x,
                                epoch=self.current_epoch, step=self.current_step,
                                on_epoch=True)
                self.logger.log('lr', self.optimizer[1].lr,
                                epoch=self.current_epoch, step=self.current_step,
                                on_epoch=False)
            ...
            self.logger.log('train_loss', loss,
                            epoch=self.current_epoch, step=self.current_step, 
                            on_epoch=True)
            self.logger.log('lr', self.optimizer.lr,
                            epoch=self.current_epoch, step=self.current_step,
                            on_epoch=False)
    ...
    def validation_step(self, batch):
        ...
        self.logger.log('val_loss', loss, epoch=self.current_epoch, step=self.current_step)
        self.logger.log('val_acc', acc, epoch=self.current_epoch, step=self.current_step,)
    ...
    def configure_optimizers(self):
        ...
        if self.config['optimizer'] == 'STP':
            ...
            return optimizers_dict[self.config['optimizer']](self.model.params, **optimizer_hparams)